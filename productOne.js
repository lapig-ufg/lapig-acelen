...
//var procRun = require('users/Amazonas21/AppSaoMartinho:InterfaceFiles/runprocess')
var func = require('users/Amazonas21/filesInterface:functions')
var datasets = require('users/Amazonas21/filesInterface:datasets')
var options = require('users/Amazonas21/filesInterface:styles')
...
var app = {
    ...
    //Funções de intereção no mapa do app pelo usuáqio
    functions:{
      ...
      guis: function(maplayer){
          ...
          var select_area = ui.Textbox({
            ...
            onChange:function(value){
                ...
                maplayer.addLayer(data,{color:'cyan'},nomeCamada)
              }
            })
          ...
          select_fonte = ui.Select({
              ...
              onChange:function(value){
                  ...
                  years = {}
                  var yearsbands = datasets.Imagescluster['Brazil'].bandNames().getInfo()
          ...
          var button = ui.Button({label:'Gerar a classificação do ano de análise'})
              button.onClick(function(){
                ...
                if (app.options.activeLegend == 0){
                      func.createLegend(maplayer,options.classes[fonteInfo],options.palette[fonteInfo])
          ...
          buttonPopUp = ui.Button({
                                label:'Informação da camada',
                                imageUrl: func.IconName('info'),
                                disabled:true,
                                style:{
                                    position:'top-left',
                                    fontStyle:'bold',
          ...
          buttonSeries = ui.Button({
                                label:'Gráfico Série Temporal',
                                imageUrl: func.IconName('timeline'),
                                disabled:true,
                                style:{
                                    position:'top-left',
                                    fontStyle:'bold',
          ...
          btnFilter =  ui.Button({
                                  ...
                                  imageUrl: func.IconName('filter_alt'),
                                  disabled:true,
                                  style:{
                                    position:'top-left',
                                    fontStyle:'bold',
          ...
          btnFilter.onClick(function(){
            ...
            if (app.options.activeBtnFilter == 0){
                ...
                layers.forEach(function(lyr){
                    ...
                    if (namelyr.indexOf('Mapbiomas') != -1){
          ...
          btnFarm.onClick(function(){
            ...
            if (app.options.activeBtnFarm == 0){
                ...
                var selectFarm = ui.Select({
                  ...
                  items:fields.getInfo()//['Fazenda','Municipio','Quadra (código)']
                  
                })
                
                var nameFarm = ui.Textbox({
                ...
                var btnFilterFarm = ui.Button('Filtrar')
                    btnFilterFarm.onClick(function(){
                        ...
                        var featFarm = ee.FeatureCollection(select_area.getValue())
                            featFarm = featFarm.filter(ee.Filter.eq(selectFarm.getValue(),nameFarm.getValue()))
                        print(selectFarm.getValue())
                        print(nameFarm.getValue())
      ...
      seriesValuesClass:function(maplayer,dataset,panelChart,active,selectCharttype){
            ...
            if (active){
              ...
              maplayer.onClick(function(coords){
                ...
                var dataValue = datasets.Dataset['Área de Restauração']
                                .filter(ee.Filter.eq('estado',dataset))
                    dataValue = dataValue.filter(ee.Filter.bounds(points))
                
                var datasource = select_fonte.getValue()
                
                var nameDataset = datasource+"_"+dataset
                    nameDataset = nameDataset.replace(' (GO)','').replace(' (SP)','')
                    nameDataset = nameDataset.replace(/ /g,"_")
  
                //var total = datasets.Imagescluster[dataset]
                var total = datasets.Imagescluster['Brazil']//ee.Image('projects/saomartinho/assets/'+nameDataset)
